...
def generate_focus_oversampled_train_list(
    ...
    if oversample_repeat <= 1 or not focus_class_indices:
        report = {
            "enabled": False,
    ...
    for line in original_lines:
        ...
        if hit_classes:
            expanded_lines.extend([line] * (oversample_repeat - 1))
...
def build_model(runtime):
    ...
    model_name = model_cfg.get("name") or runtime.get("model_name") or "FastSCNN"
    if model_name != "FastSCNN":
        raise ValueError(f"Only FastSCNN is supported in this experiment, got {model_name}.")

    return pdrs.tasks.seg.FastSCNN(
...
def configure_model_losses(model, runtime, train_stats):
    ...
    aux_loss_weight = float(loss_cfg.get("aux_loss_weight", 0.4))
    ...
    use_default_losses = (
        ...
        and abs(aux_loss_weight - 0.4) < 1e-12
    )

    if not use_default_losses:
        def build_branch_loss():
            ce_loss = seg_losses.CrossEntropyLoss(weight=class_weights)
            if use_mixed_loss:
                return seg_losses.MixedLoss(
        ...
        model.set_losses(
            [build_branch_loss(), build_branch_loss()],
            weights=[1.0, aux_loss_weight],
        )

    return {
        "use_default_fastscnn_losses": use_default_losses,
...
def build_runtime_train_config(runtime, effective_train_list, oversample_report, loss_report, warmup_steps):
    ...
    return {
        ...
        "backbone": runtime["model"].get("backbone"),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="FastSCNN training framework for GID-15 with conservative best-model export and resume support."
    )
...
def main():
    ...
    num_steps_each_epoch = max(1, train_dataset.num_samples // training_cfg["train_batch_size"])
