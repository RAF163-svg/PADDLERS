...
def compute_confusion_metrics(confusion_matrix, class_names):
    ...
    class_f1 = np.array(
        [
            _safe_divide(
                2.0 * class_precision[idx] * class_recall[idx],
...
def export_prediction_artifacts(
    ...
    if not export_predictions and not export_visualizations:
        return []
        ...
        manifest.append(
            {
                ...
                "mask": str(mask_path),
                "prediction": str(pred_path) if export_predictions else None,
                "visualization": str(vis_path) if export_visualizations else None,
                "visualization_type": vis_kind if export_visualizations else None,
            }
        )
