...
def resolve_public_path(path_value: str) -> Path:
    path = Path(path_value)
    if str(path).startswith("/public/"):
        return (Path("${NAS_ROOT}") / path.relative_to("/public")).resolve()
    ...
    with mapping_path.open("r", encoding="utf-8", newline="") as file:
        ...
        for row in reader:
            if sample_path_source == "input":
                image_path = str(Path(row["input_image"]).resolve())
                mask_path = str(Path(row["input_mask"]).resolve())
            else:
                image_path = str(Path(row["output_image"]).resolve())
                mask_path = str(Path(row["output_mask"]).resolve())
